...
def login():
    if request.method == 'POST':
        ...
        elif request.form['password'] != res[0][1]:
...
def show_update_detail():
    ...
    if request.method=='POST':
        ...
        qry_trans = "select distinct Transaction.Patient_ID, Transaction.Donor_ID, Organ_ID, Date_of_transplant, Status from Transaction, Patient, Donor where (Patient.User_ID = %s and Patient.Patient_ID = Transaction.Patient_ID) or (Donor.User_Id= %s and Donor.Donor_ID = Transaction.Donor_ID)" %((request.form['User_ID']),(request.form['User_ID']))
        #
        res_pat = ()
        res_dnr = ()
        res_trans = ()
...
def del_patient():
    ...
    organ_req = request.form.get('organ_req')

    qry = "DELETE FROM Patient WHERE Patient_ID = %s AND organ_req = %s"
    print(qry)

    try:
        mycursor.execute(qry, (patient_id, organ_req,))
        mydb.commit()
    except:
        print("Error in deletion")
...
def del_donor():
    ...
    organ_donated = request.form.get('organ_donated')

    qry = "DELETE FROM Donor WHERE Donor_ID = %s AND organ_donated = %s"
    try:
        mycursor.execute(qry, (donor_id, organ_donated,))
        mydb.commit()
    except:
        print("Error in deletion")
...
def del_doctor():
    ...
    try:
        mycursor.execute(qry, (doctor_id,))
        mydb.commit()
    except:
        print("Error in deletion")
...
def del_organization():
    ...
    qry = "DELETE FROM Organization WHERE Organization_ID = %s"
    try:
        mycursor.execute(qry, (organization_id,))
        mydb.commit()
    except:
        print("Error in deletion")
...
def del_organization_head():
    ...
    qry = "DELETE FROM Organization_head WHERE Organization_ID = %s AND Employee_ID = %s"
    try:
        mycursor.execute(qry, (organization_id, employee_id,))
        mydb.commit()
    except:
        print("Error in deletion")
